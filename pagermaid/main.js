...
const setupPagermaidIntegration = require('./modules/pagermaid');

const port = parseInt(process.env.HTTP_LISTEN_PORT || '30000', 10);

const server = http.createServer((req, res) => {
    const path = require('url').parse(req.url, true).pathname;
    // Pagermaid handles its own routing via WebSocket + Express routes
    res.writeHead(404);
    res.end('Not Found');
});

server.listen(port, '127.0.0.1', async () => {
    console.log(`[pagermaid] HTTP+WS listening on ${port}`);
    // Create fake express app for pagermaid integration
    const fakeApp = {get: () => {}, post: () => {}};
    await setupPagermaidIntegration(server, fakeApp);
});
